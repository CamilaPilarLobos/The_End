...
const UpdateAlbum = ({me, listaAlbums, setListaAlbums, logOut, setLogin}) =>{
    const {id} = useParams();
...
const sendData = (e) =>{
    e.preventDefault()
        const URL = `http://localhost:8090/api/albums/update/${id}`
        axios.put(URL,data, {headers : {token_user : localStorage.getItem("token")}}).then(
            response => {
                setlistaForos([...listaAlbums, response.data])
                navigate('/albums')
            }
        ).catch(
            e => {
